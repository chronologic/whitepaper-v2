\documentclass{report}
  ...
  \maketitle
  \begin{abstract}
    The current genration of blockchain protocols are lacking the crucial temporal functionality of transaction
    scheduling. We discuss in the following paper the potential solutions and challenges for implementing such a
    functionality on the Ethereum blockchain. (TODO add more)
  \end{abstract}
  \newpage
  \tableofcontents
  \newpage
  ...
  We schedule our routine bank transfers and bill payments; as responsible consumers 
  we know ahead of time when a payment should go through and we set it up to pay automatically
  instead of waiting for the exact time and date. As financial transactions are increasingly moving 
  to the self-sovereign and trustless world of cryptocurrencies, we see more volume is moved on networks 
  like Ethereum\footnote{\url{https://www.ethereum.org/}}. Users have more expectations about the foundational
  tooling which is similar to what the fiat world provides. Most importantly, scheduling is a feature  that 
  users expect from the Ethereum ecosystem since they have learned to rely on it so thoroughly in their everyday lives.
  We predict that scheduling calls on Ethereum will be more powerful than scheduling in the fiat world. Protocols such as
  the Ethereum Alarm Clock unlock the potential for  users to schedule smart contracts execution. Regular users now have
  access to much more complex logic. We introduce an experience similar to what developers are used to using
  cron jobs on Unix. However (and contrary to intuition) scheduling calls on a decentralized peer-to-peer
  blockchain is not a trivial problem to solve.
